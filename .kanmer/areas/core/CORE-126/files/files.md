...
| packages/core/src/store.ts | Require a concrete shared worktree before any batch declaration write; enforce the real batch actor; transact declaration plus first take; derive and recover legal after-images from a hash-bound pending WAL; retain the compact authoritative active/releasing manifest; use warning-aware censuses/direct member reads; guard pending mutations and member deletion; bind renewal to actor; refuse member transfer/reconciliation recovery; report manifest-driven state; and make every-original-member terminal release crash-safe. |
...
| packages/mcp-server/src/execution-packet.ts | Refuse pending/inconsistent batch state and any foreign-actor packet, validate projected physical Git state, and treat the authorized manifest actor/run—not display labels—as batch authority while leaving isolated resume behavior unchanged. |
...
| packages/mcp-server/src/smoke.mjs | Prove real-actor admission, worktree-required zero-write refusal, projected physical safety, display-label behavior, recoverable declaration, batch summary/archived discovery, full-terminal release and the unchanged 41-tool roster. |
| plugins/kanmer/skills/kanmer-auto/SKILL.md | Carry the automation ledger's immutable run_id as controller_run through batch declaration/recovery, packets, later-member takes and every CAS renewal. |
| plugins/kanmer/skills/kanmer-execute/SKILL.md | Keep one footer per exact frozen member, require the shared worktree/controller rule, create the PR once, and make later members validate/reuse/record that exact PR. |
| plugins/kanmer/skills/kanmer-review/SKILL.md | Tell one fresh reviewer how to review the shared exact head once, write a member-owned attestation for every roster ticket, and idempotently advance the complete roster after the confirmed shared merge. |
