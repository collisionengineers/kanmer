...
## CORE-088 child remediation handoff — 2026-08-22

CORE-088 implements F-006 through F-009 from the cumulative source review: symlink-safe/bounded cache validation and origin checks, root-304 validator persistence and forced-refresh retry, Node DNS scalar/`all:true` pinned lookup, stale owner-marker/future-lock/error aggregation hardening, and locked orphan quarantine/fingerprint cleanup. Commit `8d62176216d8c886779217fd846149f0b04b1655` is on branch `core-088-source-cache-atomicity`, targeting `core-026-project-declared-sources`. Deterministic rails are sources 32/32, core IO 32/32, GUI Git 31/31, all-workspace typecheck exit 0, docs exit 0, and plugin build pass. Plugin check is INCONCLUSIVE in the linked worktree and mcpb check is INCONCLUSIVE because the local mcpb CLI module is absent. CORE-026 remains Review/blocked pending independent child review; no parent merge or stage change was made.
