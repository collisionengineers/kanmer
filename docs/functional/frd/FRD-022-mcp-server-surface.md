...
- R1 — **37 tools registered** (recounted from `registerTool` call sites, including source
  and execution-packet surfaces): the 15 reads, 20 writes, and 2 destructive. The count reached the end state via the six group tools —
...
- R6 — the rail is real: `plugin:check` compares every registered tool name and
  matching bundle bytes, while `smoke.mjs` and `smoke-protocol.mjs` exercise the
  current surface and annotations. The exact counts are generated by those rails,
  not frozen prose.
