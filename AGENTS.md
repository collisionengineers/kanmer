...
lease_workspace: worktree:…  # │ its lease on first renew/transfer (one migration path).
lease_phase: implementing    # ┘
lease_batch: batch-x         # ┐ frozen batch membership (FRD-030 batch mode, CORE-124):
lease_batch_frozen_at: …     # ┘ stamped on every member by the first member take; absent when isolated
labels: [mcp]
...
17. **A claim is a lease, and every ticket write goes through one lock.** `takeTicket`/`renewTicket`/`transferTicket`/`releaseTicket` run under `withExclusiveFileLock(.kanmer/leases.lock)` and re-read the ticket inside it, so the revision CAS and the write are one step (CORE-114 F-009) and the sibling scan that keeps **one live writer per workspace** (`WORKSPACE_OCCUPIED` on a worktree or branch another taken ticket records — `force` does not bypass it) is race-free. Do not add a lease verb that writes outside `withLeaseLock`. **Since CORE-125 that lock is the board's write lock, not the lease verbs' private one:** `updateItem` (and therefore `moveItem`), `setDoc` and `appendScratch` also run their locate → read → CAS → write inside it, so an ordinary field edit can no longer rename over a lease record (CORE-115 F-001) or a sibling's `lease_batch` stamp (CORE-124 F-001). Any new ticket-file writer belongs inside it too. `withLeaseLock` is re-entrant **within one async execution context only** (an `AsyncLocalStorage` set of held lock paths), because `updateItem` → `appendTransition` → `setDoc` and the lease verbs' own transition writes nest — `withExclusiveFileLock` itself is not re-entrant and a naive nested acquire self-deadlocks until its retry budget throws. Keep the section tight: `moveItem` deliberately leaves `assertMoveAllowed` and `computeOrder` (which can rewrite a whole column) outside the lock, and nothing slow, networked or git-shaped belongs inside it. `leaseState()` is the only expiry rule: a taken ticket without `lease_id` is a v0.3.12/CORE-121 legacy claim (expires `taken_at + claimExpiryMinutes`) and receives its lease record on its first renew or transfer — never add a second ownership model or a `migrate_board` step for it. `transfer` is the reclaim: the MCP boundary collects `ReconciliationEvidence` first (core is git-free), the store records the summary in `scratch/execution.md` and `lease_reclaimed_from`, refuses `RECOVERY_REFUSED` for a board/foreign/branch-mismatched workspace, and never deletes anything — an expired lease still owns its workspace until `release` at closeout. Renewal names `lease_id` + `lease_revision` (`LEASE_EXPIRED` / `REVISION_CONFLICT`); a renew naming no lease is the compatibility lane for installed skills and applies the owner check only. Lease fields are additive frontmatter (passthrough), so the live v0.3.12 board reads them unchanged.
18. **A batch is the only exception to one-writer-per-workspace, and it is frozen by its first take.** FRD-030 batch mode (CORE-124) is two additive fields on every member — `lease_batch` and `lease_batch_frozen_at` — written only by `takeTicket` inside `withLeaseLock` when the first member passes `batch` + `batchMembers` (the complete list, validated before any write: `BATCH_INVALID` / `BATCH_FROZEN`). There is no batch file, no `update_item` path and no new tool; membership *is* the set of tickets sharing the id. `assertWorkspaceFree` lets a same-batch sibling share the recorded worktree and branch and refuses anything else for a member (`BATCH_WORKSPACE_MISMATCH`), while a non-member still gets `WORKSPACE_OCCUPIED`; `get_execution_packet` mirrors that exception in its physical worktree scan and reports `claim.batch`. Documents stay per ticket (one PR/head attestation referenced by every member, one proof each); `releaseTicket` — the GUI's release path too — refuses `BATCH_ACTIVE` until every member is Done or archived (`isTerminalTicket`), and the last release clears the record. Sibling stamps append no activity ops, so `store.test.ts`'s take/release op sequence is unchanged.
19. **Plan validation flags; only a `step` request blocks.** FRD-033 (CORE-118) adds two pure core modules — `plan.ts` (`parsePlan`/`validatePlan`, plus the ATX section reader that used to live in `execution-packet.ts`) and `step-packet.ts` (`compileStepPacket`, `step-packet/1`) — and one optional `step` parameter on `get_execution_packet`. There is **no new tool** (CORE-118 added none; the roster moved to 40 only when CORE-131 added `apply_reconciliation`), no new frontmatter field, no persisted packet and no `store.ts` change; a packet is derived on read. The severity split is load-bearing: `PLAN_VAGUE_INSTRUCTION` and `PLAN_RISK_EVIDENCE_MISSING` are permanently **advisory**, because the shipped plan template says the decision-verb warning "is not a gate or regex score" and `scripts/verify-skill-prose.mjs:347-348` pins that sentence — do not promote them. Structural findings (no compilable step, an allowed file the `## Expected files` table never declares, a file `## Do not modify` forbids, no usable acceptance check, no stop condition, evidence pinned to a superseded version) become blockers **only** when `step` is supplied, which is what keeps every existing plan's whole-ticket packet working unchanged. The step refusal is strictly **last** in the refusal order and, like every other, writes nothing — `smoke.mjs` proves it by byte-comparing `.kanmer`, the ticket file and `activity.jsonl` across it. `"next"` reads a checklist by the step each box names (`- [ ] Step 2 — …`) and only falls back to box position, because real checklists carry more boxes than steps.
