...
12. **Electron Builder packages; it does not publish.** v0.3.8 proved its concurrent GitHub publisher can race release creation and leave a partial public release. `release.mjs` therefore runs one `--publish never` package, validates that exact directory, creates a draft GitHub Release, uploads exactly installer/blockmap/MCPB/manifest, and compares GitHub digests with the retained files before making the coherent draft public/latest. It refuses existing tags/releases and never repairs or clobbers a failed immutable version. Independent tag CI still builds the package as a source-health check, but signed NSIS output is not reproducible across builders; CI uses `--remote-coherent` to bind the public `latest.yml` to the downloaded public installer instead. A missing asset or digest remains a hard failure.
