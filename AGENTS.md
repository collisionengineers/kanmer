...
4. **The installed app runs the MCP server as Electron-as-Node.** `connect.ts` registers `command = <Kanmer.exe>`, `args = [<resources/mcp/kanmer-mcp.cjs>, --root, <project>]`, `env = { ELECTRON_RUN_AS_NODE: "1" }` for project-scoped registrations. The native Antigravity plugin instead uses the installer-owned Windows launcher named by `%LOCALAPPDATA%\\Kanmer\\bin\\kanmer-mcp.cmd`; its `cmd.exe` descriptor is portable and does not require a separate Node install. The standalone bundle must therefore be self-contained (no node_modules at runtime).
