...
| packages/core/src/types.ts, board.ts, store.ts, io.ts | Core is the single schema/store/atomic-write authority; preserve unrelated board fields and use the serialized board-update lock for source patches. |
