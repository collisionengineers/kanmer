---
kind: auto-current
schema: 1
run_id: 20260821T105846Z-codex-mcp-client
run_path: automation/runs/20260821T105846Z-codex-mcp-client.md
group: HZN-007
project_fingerprint: C:\Users\Alex\Documents\GitHub\kanmer\.worktrees\kanmer|repo=C:\Users\Alex\Documents\GitHub\kanmer|format=3|server=a35e1fd0
controller: codex-mcp-client
status: running
updated_at: 2026-08-21T22:15:54.565Z
---

# Current auto run — HZN-007

## Resume instruction

Open `automation/runs/20260821T105846Z-codex-mcp-client.md`, then re-read HZN-007/context.md, the live roster, and get_doc_gates for every ticket before dispatching. This pointer is written only after the complete immutable run record was written and read back.


Last handoff: CORE-032 is merged and intentionally held in Verifying because the real Windows verify check is red in the pre-existing kanmerGit path assertion; proof and review disposition are recorded. GUI-068 and MCP-020 remain active implementation lanes; resume from the live roster and get_doc_gates before any next dispatch.
