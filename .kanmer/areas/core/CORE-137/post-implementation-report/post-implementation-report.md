...
Steps 1–5 of the plan and review-round-1 remediation are done; steps 6–11 (delta review, merge, publish, release verification, promotion acceptance, channel completion) follow and are recorded in `scratch/promotion.md` and `proof/proof.md`.
...
## Files changed (release PR #319, head `daaf95fa234cc0ce87cd5dd83d556cad6610fdfc`)
...
| `daaf95fa docs(release): correct v0.4.1 notes after review` | `apps/gui/release-notes.md`: correct registration/gitignore guidance and document the review-gate and discovery behaviour omitted from the first draft |

All hand edits are confined to `apps/gui/release-notes.md`; the version and bundled-artifact changes are release-script output.

## Commands and exit codes

| Command | cwd | Exit | Result |
|---|---|---|---|
| `git clone … kanmer-release-0.4.1` + `npm ci` | fresh clone at `04a97751` | 0 / 0 | `C:\kt-tmp\core137\clone.log` |
| `bash notes-commit.sh` (insert + commit notes) | clone, local `main` | 0 | commit `4fd6998a` |
| `npm run release -- 0.4.1 --ticket CORE-137` | clone | 0 | 25 script commands: the shared rail (21 rail steps incl. golden step `20/20 scenarios passed in 16786 ms (budget 300000 ms)`), bump, bundle + MCPB rebuild, `plugin:check` → `plugin-sync OK — 41 tools match, bundle bytes match, 12 skill frontmatters parse, manifests at v0.4.1, isolated MCP handshake lists 41 tools`, GUI build, commit, push `release/v0.4.1`, PR (`C:\kt-tmp\core137\prepare.log`) |
| `npm run plugin:check` | release clone at `daaf95fa` | 0 | `plugin-sync OK — 41 tools match, bundle bytes match, 12 skill frontmatters parse, manifests at v0.4.1, isolated MCP handshake lists 41 tools` |
| `git diff --check` | release clone before remediation commit | 0 | no output |
| `git push origin release/v0.4.1` | release clone | 0 | `6637239d..daaf95fa` |
...
The independent whole-file review returned needs-changes at `6637239d2af844f9fae4c59bc572ec6535341509`.

- F-001 (blocker), fixed: the upgrade guidance now says the registration is per-machine, remains gitignored and must not be committed; only the refreshed managed block and `.gitignore` entries are committed.
- F-002 (major), fixed: the notes now state that every open finding, including minor and note, blocks the merge gate until terminal disposition.
- F-003 (major), fixed: the notes now explain that Claude Code and OpenCode rely on discovery without `--root`, identify the supported project/`.kanmer` and project/`.worktrees` locations, and warn that Connect reports an out-of-tree board.

These three changes form the single release-note remediation commit `daaf95fa234cc0ce87cd5dd83d556cad6610fdfc`. The ticket plan and `scratch/promotion.md` were also repaired as board records to replace malformed control-character paths and to make the post-merge local-main realignment safe; they are not PR files.
...
- The stored execution packet rejects the standalone release clone as a foreign repository. The operator explicitly authorized resuming that recorded workspace after inspection confirmed it is the clean PR source clone, and no second worktree or branch was created.
