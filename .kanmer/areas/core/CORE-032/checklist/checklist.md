...
- [ ] Confirm the current PR head receives exactly one check named verify. Real GitHub evidence is not yet available.
- [ ] Confirm the run uses Windows, Bash, Node 20, and executes both npm ci and npm run verify successfully. Real GitHub evidence is not yet available.
- [ ] Record the run ID, head SHA, conclusion, and elapsed duration; note whether it meets the under-ten-minute target. Real GitHub evidence is not yet available.
...
- Real PR/Actions and post-merge kanmer-board non-trigger evidence remain INCONCLUSIVE until the PR is opened and merged by the independent review/verification stages.
