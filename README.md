...
```
.kanmer/
  data/board.yml       Phases, statuses (kanban columns), id prefixes
...
- **Board** — phases are swimlanes, statuses are columns. Drag cards to move; within each column cards **cluster by area** under colour-coded sub-labels and carry an area stripe.
- **Editor** — click a card to edit every frontmatter field (incl. **area**, configurable **priority**) and the Markdown body; `[[ID]]` gets **autocomplete**; **Archive** hides an item, and Delete asks first.
- **Search + filter bar** — filter by area, priority, assignee, label; toggle archived.
- **Settings** (gear) — add/rename/recolour/reorder/delete **phases, statuses, areas, priorities**, edit **id prefixes**, and switch **theme** — all written to `board.yml`/app settings, reflected instantly. No file editing required.
...
Read: `list_phases`, `list_items`, `get_item`, `search_items`, `get_links`
Write: `create_item`, `update_item`, `move_item`, `link_items`, `add_column`
Destructive: `delete_item`

Items carry a configurable `area` (colour-coded, groups cards on the board) and `priority`, and can be `archived` (hidden from the board, via `update_item`). `add_column` manages phases, statuses, areas and priorities.
