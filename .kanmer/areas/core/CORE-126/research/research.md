...
- CORE-132 provides the proven local pattern: persist a strict write-ahead intent before a multi-file mutation, roll the exact intent forward after interruption, and fail closed on a conflicting or malformed journal. CORE-126 should reuse that pattern and existing writeFileAtomic/withLeaseLock helpers, not the release-record types or a generic transaction engine.
- A short-lived journal under a batch sidecar folder is invisible to the v0.3.12 item scan, which walks areas only. Hashing the batch id for the filename avoids path traversal and Windows filename collisions; the record retains the exact id, controller, frozen timestamp, and sorted roster.
...
- CORE-125 already serializes the relevant ticket writers through withLeaseLock. That contract must be preserved; no new unlocked writer, stage, database, service, or workflow surface is needed.
...
- Add a strict recoverable batch-declaration intent and lease_batch_controller. Matching same-actor/exact-roster retries roll forward idempotently; a different actor, roster, malformed intent, or contradictory member bytes is retained and refused.
