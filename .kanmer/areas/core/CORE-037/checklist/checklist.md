...
- [ ] Add the test-local native/lexical `pathIdentity` helper without changing production code.
- [ ] Apply `pathIdentity` only to the three existing-worktree path identity assertions; retain all real Git, branch, ref, and missing-path assertions.
- [ ] Run the focused `kanmerGit.test.ts` suite and record the exit/count plus any baseline cleanup EPERM or hook timeout.
