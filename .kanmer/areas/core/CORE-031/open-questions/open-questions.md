...
- [x] **What is the authoritative command order?** — Use, in order: `npm test`; `npm run typecheck`; `npm run build`; `node packages/mcp-server/src/smoke.mjs`; `npm run smoke:protocol`; `npm run smoke:discovery`; `npm run verify:skills`; `npm run verify:agents-block`; `npm run plugin:check`.
