...
| `reconcile_ticket` | Read-only, dry-run-first recovery inspection for one existing ticket. It gathers the fixed board, proof, recorded-workspace and PR/check facts, returns deterministic findings plus one safe proposal or no proposal, and never mutates the board, Git, workspace, release state or board worktree. Unavailable GitHub/CI/workspace facts are explicit no-apply evidence, not success. | `id` |
...
| `apply_reconciliation` | Apply only the exact current proposal returned by `reconcile_ticket`. The server re-collects facts, requires the prior ticket revision and proposal fingerprint to match, then performs only an existing legal stage move or clean terminal claim release. It cannot accept a path, command or provider, and never deletes a worktree, bypasses checks or mutates the board worktree. | `id`, `expected_updated`, `proposal_id`, `expected_project?` |
