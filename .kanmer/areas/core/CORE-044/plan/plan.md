...
2. **Make board source writes optimistic.** Add a board content/version token at the core/store seam and require set_sources to supply/check it alongside expected_project. Ensure a stale whole-board read returns a conflict without overwriting unrelated GUI/agent edits; add two-writer tests and retain all existing board fields.
