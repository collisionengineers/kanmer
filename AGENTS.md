...
4. **The installed app runs the MCP server as Electron-as-Node.** The fixed installer-owned launcher is the portable registration boundary: installs stage the complete Electron runtime tree (exe, sibling DLLs/resource packs, app resources, standalone bundle and skills) under an immutable `%LOCALAPPDATA%\Kanmer\mcp\<version>-<installer-pid>` generation (the PID suffix makes same-version repair non-overwriting), then rename its executable to `kanmer-mcp.exe`. Copying only the exe, ICU and V8 snapshot fails at process start with Windows `0xc0000135`. The script remains at `<runtime>/resources/mcp/kanmer-mcp.cjs` and bundled skills at `<runtime>/resources/plugins/kanmer/skills`. It activates a stable `current` junction only after the generation is complete and preserves the install-root `Kanmer.exe`/`resources/mcp/kanmer-mcp.cjs` payload as a legacy fallback. Project-scoped `connect.ts` registrations may still invoke the install-root `Kanmer.exe` directly with `ELECTRON_RUN_AS_NODE=1`; the native Antigravity plugin instead uses the portable `%LOCALAPPDATA%\Kanmer\bin\kanmer-mcp.cmd` launcher, which does not require a separate Node install. Codex Connect probes that launcher through `cmd.exe /d /s /c call "<portable launcher>" --probe` with `windowsVerbatimArguments`: `call` keeps `/s /c` from stripping a quoted first token and verbatim arguments keep Node from turning the embedded quotes into literal `\"` characters. The persisted Codex registration remains the rootless launcher command required by FRD-012; the `call` form is probe-only. The standalone bundle must therefore be self-contained (no node_modules at runtime).
...
10. **An update force-kills legacy install-root MCP servers — and if it doesn't, the update fails.** Electron Builder 26's stock PowerShell predicate queries nonexistent `Win32_Process.Path`, so it silently matches nothing; `apps/gui/build/installer.nsh` therefore supplies the supported `customCheckAppRunning` override and uses `ExecutablePath` under the exact, trailing-separator `$INSTDIR` boundary. It rechecks after bounded graceful/forced stops and refuses before uninstall on an error or remaining process. Direct/legacy registrations still run under `$INSTDIR`, while the fixed launcher provisions fresh sessions from the external runtime under `%LOCALAPPDATA%\Kanmer\mcp`, outside that boundary. electron-updater passes `--updated`, so the installer gives the parent a bounded exit grace and then clears legacy children without an extra prompt. This is why `mcp-sessions.ts` still exists: it names what dies before the user commits to the update. Failing open (`unknown: true`) remains deliberate for the *warning*; installer clearance itself fails closed.
...
- **Release verification is independent CI; publishing remains local.** `.github/workflows/release.yml` runs one read-only `release-verify` job on each `v*` tag push. On `windows-latest` it checks out the pushed tag, validates the root/GUI/plugin versions, runs `npm run verify`, then builds core/server and GUI before running `npm run dist -w @kanmer/gui -- --publish never` and `node scripts/check-updater-package.mjs`; it then polls `node scripts/verify-release-assets.mjs <version>` for a bounded period. It has `contents: read` only and never publishes or repairs releases; `npm run release <version>` remains the sole publisher and repair owner.
