...
- [ ] Step 2 — Amend ADR-0011, add board-owned report/strict proof policy, wire the shared parsed state into the existing gate engine, refuse generic-writer strict escalation, and update every affected fresh-board strict fixture.
- [ ] Step 3 — On current-format boards only, extend `migrate_board` with a complete byte-preserving census/digest and one dedicated lock-atomic digest-bound strict-policy cutover method.
...
- [ ] Step 5 — Update ADR-0011, FRD-002/FRD-006 and canonical verify/closeout/auto/setup/manual prose, regenerate manual/bundle, run focused/full rails and open one bounded PR.
...
- [ ] Old-format/incomplete census, missing/stale digest, concurrent drift and generic board-writer escalation refuse without writes; successful current-format cutover changes only board proof policy under the same lock.
...
- [ ] ADR-0011 authorizes the bounded proof reader; FRD-002, FRD-006, setup and proof/gates/first-ticket manuals match report/strict behavior; no new tool/stage/dependency appears and the plugin roster remains 41.
