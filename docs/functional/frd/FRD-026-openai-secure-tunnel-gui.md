...
- **R1 — Project isolation.** A profile belongs to one canonical project
  ...
  boards behind one profile and must reject duplicate runtime aliases, client
  profile names, or loopback health addresses across projects.
...
- **R4 — Managed lifecycle.** Kanmer manages one tunnel-client runtime alias per
  ...
  Disabling a profile stops and confirms its persistent alias first. Identity
  reconciliation stops and removes the old local alias before transferring the
  profile, so no runtime remains bound to stale board or repository roots.
- **R5 — Honest health.** Missing executable, missing named environment
  ...
  Checking an existing runtime does not require the connect credential. Ready
  is claimed only when structured runtime status reports `process_running`,
