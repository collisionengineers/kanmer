...
reviewed once; the attestation and post-merge proof remain per member.
...
18. **A batch is the only exception to one-writer-per-workspace, and its manifest is the authority.** FRD-030 batch mode keeps the three ticket projections (`lease_batch`, `lease_batch_controller`, `lease_batch_frozen_at`) plus one hash-named sidecar under `.kanmer/batches/transactions/`; it adds no tool, stage, database or workflow engine. The first `takeTicket` holds `withLeaseLock`, validates the complete two-or-more-member roster from one warning-aware census, and writes a hash-bound `pending` write-ahead record before changing any endpoint. The same exact declaring request can roll every member from its observed hash to its derived intended hash and compact the record to `active`; contradictory bytes, unreadable tickets, another actor, a partial/extra roster or another request fail closed while preserving the manifest. `lease_batch_controller` is the actual transport actor — `assignee`, `controller` and observable owner strings never authorize take, renew or declaration recovery — and batch transfer/reconciliation recovery is refused. `assertWorkspaceFree` lets only that controller's member share the exact branch/worktree; a non-member still gets `WORKSPACE_OCCUPIED`. The PR carries the exact complete footer roster, every member is in Review, and every member owns its independent-pass exact-PR/exact-head attestation and post-merge proof. Cleanup begins only after the immutable roster is entirely Done or archived: `releaseTicket` advances `active` → `releasing`, clears members idempotently across interruption, removes the manifest last, and only then may closeout delete the shared worktree and branch.
