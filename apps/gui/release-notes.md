...
After a worker returns from a step packet, Kanmer now compares what actually changed in the worktree, which document versions moved and which checklist marker was ticked against the packet that authorised them. A forbidden or undeclared path, a stale plan or evidence version, a skipped or pre-ticked step, or an unobservable workspace produces a typed finding — forbidden, undeclared, stale, or inconclusive — and the next packet is refused until the previous one reconciles PASS. Declared paths are parsed strictly (literal, segment `*`, whole-segment `**`), and a bare top-level entry such as `LICENSE` under "Do not modify" is honoured. The inspector stays read-only.
