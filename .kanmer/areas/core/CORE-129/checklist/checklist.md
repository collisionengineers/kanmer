...
> (with `origin/main` `9945b1f2` merged). Every box below is ticked against a named
> test or a recorded command; see `post-implementation-report/`.
...
- [x] `WAIVED_BY_OPERATOR` is accepted only at the top level and only with the operator identity fields; without them it is `invalid`.
...
- [x] In `strict` mode the proof requirement is satisfied only by `valid-pass`; legacy, invalid, FAIL, INCONCLUSIVE and waived records block Done.
