...
8. **Publish.** In the clean release clone, while still on `release/v0.4.1`: fetch `origin/main`; require PR #319 to be `MERGED` with a full `mergeCommit.oid`; require `origin/main` to equal that SHA; require the current local `main` tip to be an ancestor of `release/v0.4.1`; and require local branch `core137-premerge-main` not to exist. Rename local `main` to `core137-premerge-main`, create a new local `main` at `origin/main`, and switch to it. Do not use `git pull --ff-only` after the squash merge because the pre-merge local notes commit diverges from the squashed `origin/main`. Export `GH_TOKEN` from `gh auth token`; run `npm run release -- 0.4.1 --publish --release-commit <full merge SHA>`. The script builds the GUI, creates and pushes `refs/tags/v0.4.1`, packages once with publishing disabled, uploads the canonical asset set to a draft, verifies digests, then publishes. Record every exit code. Copy the four assets to `KanmerBackups\\installers\\0.4.1`.
...
10. **Promotion acceptance** (each command and exit code in `scratch/promotion.md`, summarised in `proof/proof.md`):
    ...
    j. **CORE-119 acceptance:** `npm run golden` exit 0 at the release SHA; then the copied-board rehearsal `node scripts/golden-promotion.mjs --candidate 0.4.1 --stable 0.4.0 --board-backup "C:/Users/Alex/Documents/KanmerBackups/kanmer-board-20260904T071345Z.zip" --board-copy "<copy dir>" --stable-installer "C:/Users/Alex/Documents/KanmerBackups/installers/0.4.0/Kanmer-Setup-0.4.0.exe" --candidate-installer "C:/Users/Alex/Documents/GitHub/kanmer-release-0.4.1/apps/gui/release/Kanmer-Setup-0.4.1.exe" --launcher "\"C:/Users/Alex/AppData/Local/Kanmer/bin/kanmer-mcp.cmd\" --root \"<copy dir>\"" --out "C:/kt-tmp/core137/promotion.json"` exit 0. **Binding rule (CORE-119 review F-002):** `driveCopiedBoard()` runs the `--launcher` command verbatim and never adds `--board-copy` to it, so the launcher string itself must carry `--root <copy dir>`; before the run, execute the same launcher string by hand and confirm its `get_status.project` names the copy, not the live board. A launcher without `--root` is refused by the operator, not by the script.
