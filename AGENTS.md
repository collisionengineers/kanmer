...
18. **A batch is the only exception to one-writer-per-workspace, and its manifest is the authority.** FRD-030 batch mode keeps the three ticket projections (`lease_batch`, `lease_batch_controller`, `lease_batch_frozen_at`) plus one hash-named sidecar under `.kanmer/batches/transactions/`; it adds no tool, stage, database or workflow engine. The first `takeTicket` holds `withLeaseLock`, validates the complete two-or-more-member roster from one warning-aware census, and writes a hash-bound `pending` write-ahead record before changing any endpoint. The same exact declaring request can roll every member from its observed hash to its derived intended hash and compact the record to `active`; contradictory bytes, unreadable tickets, another actor, a partial/extra roster or another request fail closed while preserving the manifest. Batch work authority is the exact pair of the actual MCP request actor and a nonempty durable `controller_run` retained by the controller across reconnects and restarts. The pending, active and releasing manifests persist both values and every declaration, recovery, later member take, renew and execution-packet request must exact-match them; `assignee`, `controller` and observable owner strings never authorize those operations. Batch transfer/reconciliation recovery is refused. `assertWorkspaceFree` lets only that actor/run pair's member share the exact branch/worktree; a non-member still gets `WORKSPACE_OCCUPIED`. `list_items` and `search_items` project each active or releasing manifest back onto every immutable-roster member until the manifest is unlinked, including `state`, the complete `members`, `workspace` and `branch`, so a fresh closeout can recover even after all ticket-local projections were cleared. The PR carries the exact complete footer roster, every member is in Review, and every member owns its independent-pass exact-PR/exact-head attestation and post-merge proof. Cleanup begins only after the immutable roster is entirely Done or archived: terminal `releaseTicket` is deliberately not actor-bound, so a fresh closeout may advance `active` → `releasing`, clear members idempotently across interruption, and remove the manifest last. Only then may closeout delete the shared worktree and branch.
