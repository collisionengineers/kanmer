...
`batch` (`{ id, controller, frozenAt }` from the ticket-side projection of the
active manifest, or `null`; call with `include_archived: true` and capture the
complete roster before batch release begins),
...
- `lease_batch` / `lease_batch_controller` / `lease_batch_frozen_at` — the
  ...
  idempotent. `lease_batch_controller` is the actual request actor; supplied
  owner labels cannot authorize member take, renew, or declaration recovery.
  These fields are absent in isolated mode and may clear member by
  member only while the manifest is `releasing`.
...
evidence is written separately on every ticket in the immutable roster. Each
