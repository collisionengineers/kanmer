...
| `move_item` | Move an item to a workflow stage. Rejects a status that is not on the board — call `list_board` for valid ids. Moving a ticket to the **final** stage requires its `proof.md` to exist. | `id`, `status`, `expected_updated?` |
...
`area`, `priority`, `assignee`, `labels`, `created`, `updated`, `archived`,
...
(`{ checked, total }` or `null`). `links` and the Markdown body are **not**
included and require `get_item` — a `null`/absent relation in a summary means
"not reported here", not "no links".
