...
| `get_execution_packet` | Read-only weak-agent entry point: returns one bounded implementation packet or a normal `ready:false, code:GATE_BLOCKED` refusal. Refusal precedence is non-ticket/legacy → spike → unmet leave-preparing requirements → unresolved questions → occupied by another actor; `missing` contains exact raw requirements (or `[]` for occupancy). A ready packet includes project identity, ticket/body/taken details, ordered full group contexts, profile-resolved gates, fixed `plan`/`checklist`/`files` index documents with versions, sorted extra Markdown paths/versions, an ATX stop condition, and command hint. Chore tickets need only their resolved plan; same-actor occupancy may continue. The call never takes, moves, writes, dispatches or creates a worktree. | `id` |
