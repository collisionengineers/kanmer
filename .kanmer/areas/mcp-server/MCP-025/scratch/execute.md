...
Rebased PR #90 onto origin/main, fixed the production CLI to fail before binding when no authorizer integration exists, passed focused verification, marked PR ready, and moved MCP-025 to Review. Do not merge from execute.

MCP-032 remediation: committed and pushed `987fe05`. Disabled ESM code splitting so the spawned `dist/index.js` is self-contained and self-identifying; normal stdio smoke is now 175/175. HTTP smoke, protocol 30/30, and discovery 13/13 still pass. Ticket remains Review; do not merge.
