...
| `take_ticket` | Take a ticket before working it: records `taken_at` + `branch` (required) + `worktree?`, sets assignee (defaults to your client name), stamps `claim_expires_at` (board `claimExpiryMinutes`, default 30) and `claim_controller`, and moves to the working stage (default `implementing`). Errors if already taken unless `force`. `action: "release"` clears the taken fields when the work ends. `action: "transfer"` hands an **expired** claim to the caller (or `assignee`) while keeping the recorded branch/worktree — a live claim refuses with `CLAIM_LIVE` unless `reason` begins `operator:`. `action: "renew"` extends your own claim; a foreign claim refuses with `CLAIM_NOT_OWNED`. Never `force` to recover a dead controller's ticket — transfer it. | `id`, `action` (`take`/`release`/`transfer`/`renew`), `branch`, `worktree?`, `stage?`, `assignee?`, `controller?`, `reason?`, `force?` |
