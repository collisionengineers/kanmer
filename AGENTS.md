...
- Work each fresh ticket on its own branch and worktree: worktree `.worktrees/<id>`, branch `<id>-<slug>`; `take_ticket` records both and moves the stage. A resumed execution packet must validate and reuse its exact recorded branch/worktree — never create a second worktree or take the ticket again. It must not be the board, the shared source checkout, or another active ticket's worktree, and its Git common directory must match the source repository. Pause by retaining that taken record; never release a paused ticket while its worktree/branch remains a resume target.
