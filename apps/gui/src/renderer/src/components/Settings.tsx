...
export function OpenAITunnelSection({ projectId }: { projectId: string }): JSX.Element {
  ...
  return <div className="settings-section" aria-label="OpenAI Secure MCP Tunnel">
    ...
    <div className="button-row">
      ...
      {view?.identityConflict ? <button className="ghost sm" disabled={busy !== null} onClick={() => void reconcile()}>{busy === "reconcile" ? "Reconciling…" : "Reconcile identity"}</button> : <><button className="ghost sm" disabled={busy !== null || !view?.profile || draftDirty} onClick={() => void initialize()}>{busy === "initialize" ? "Connecting…" : "Connect runtime"}</button>
