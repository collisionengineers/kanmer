...
| `kanmer-research` | The research phase (**Researching** stage): investigate a ticket and write `research.md` (findings), `impact.md` (files touched), and `open-questions.md`. Read-only — no branch needed. |
...
| `kanmer-review` | Review finished work: write the 4-doc PR-review set, check the plan's Governing-docs section holds, review the PR diff, turn feedback into blocking tickets — then **merge** and move the ticket to Verifying. |
...
| `kanmer-groom` | Board-editing triage (propose, then apply): dedupe near-duplicates, fill missing areas/priorities, split oversized tickets, archive dead ones, chase stale takes, and clear **doc-gate debt**. |
