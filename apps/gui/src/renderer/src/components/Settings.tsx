  ...
  return <div className="settings-section">
    ...
    {providers.map((provider) => {
      ...
      return <div className="dispatch-provider" key={provider.id}>
        ...
        {provider.model ? <label className="field"><span>Task override</span><select value={selectedTask} onChange={(e) => setSelectedTask(e.target.value)}>{tasks.map((task) => <option key={task.id} value={task.id}>{task.label}</option>)}</select><input value={taskOverrides[selectedTask as keyof typeof taskOverrides] ?? ""} maxLength={200} placeholder="Use provider default" onBlur={(e) => updateProvider(provider.id, { taskModels: { ...taskOverrides, [selectedTask]: e.target.value } })} /></label> : <p className="hint">Task model overrides are unavailable until this provider has measured model support.</p>}
...
function ConnectSection(): JSX.Element {
  ...
  const run = async (target: ConnectTarget, action: "connect" | "disconnect") => {
    if (target === "grok" || target === "antigravity") {
      ...
      const host = target === "grok" ? "Grok" : "Antigravity";
      const approved = window.confirm(
        `${host}'s Kanmer plugin is user-scoped. ${verb === "install" ? "Installing" : "Uninstalling"} it affects every ${host} workspace for this user. Continue?`,
      );
      if (!approved) return;
    }
  ...
  return (
    <div className="settings-section">
      ...
      <p className="hint">
        ...
        via its plugin marketplace (Claude Code, Codex), a user-scoped native plugin (Grok and
        Antigravity), or a project skills directory owned by the host. Native plugin installs
        are verified before any legacy project state is cleaned; Antigravity background dispatch
        binds each session with <code>--add-dir</code>.
      </p>
...
export function OpenAITunnelSection({ projectId }: { projectId: string }): JSX.Element {
  ...
  return <div className="settings-section" aria-label="OpenAI Secure MCP Tunnel">
    ...
    <div className="button-row">
      ...
      {view?.identityConflict ? <button className="ghost sm" disabled={busy !== null} onClick={() => void reconcile()}>{busy === "reconcile" ? "Reconciling…" : "Reconcile identity"}</button> : <><button className="ghost sm" disabled={busy !== null || !view?.profile || !draft.enabled || draftDirty} onClick={() => void initialize()}>{busy === "initialize" ? "Connecting…" : "Connect runtime"}</button>
      <button className="ghost sm" disabled={busy !== null || !view?.profile || draftDirty} onClick={() => void runDoctor()}>{busy === "doctor" ? "Checking…" : "Check status"}</button></>}
    </div>
    <div className="settings-section"><h4>Status: {status?.state ?? "disabled"}</h4>{status && <p className="hint">Action <strong>{status.action}</strong> · Severity <strong>{status.severity}</strong> · Executable {status.health.executable} · Credential {status.health.credential} · Listener {status.health.listener} · MCP {status.health.mcp}</p>}{status?.restartRequired && <p className="banner warn">Reconnect this managed runtime after the downloaded update is installed.</p>}{status?.lastSummary && <p className="hint">{status.lastSummary}</p>}{status?.lastError && <p className="error-text">{status.lastError}</p>}<div className="button-row"><button className="primary sm" disabled={busy !== null || active || !draft.enabled || draftDirty || view?.identityConflict === true} onClick={() => void start()}>{busy === "start" ? "Connecting…" : "Connect"}</button><button className="ghost sm" disabled={busy !== null || !active} onClick={() => void stop()}>{busy === "stop" ? "Stopping…" : "Stop"}</button><button className="ghost sm" disabled={busy !== null || !view?.profile || draftDirty || view.identityConflict === true} onClick={() => void restart()}>{busy === "restart" ? "Reconnecting…" : "Reconnect"}</button>{view?.profile && !active && !view.identityConflict && <button className="ghost sm" disabled={busy !== null} onClick={() => void remove()}>{busy === "remove" ? "Removing…" : "Remove local runtime"}</button>}</div></div>
