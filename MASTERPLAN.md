...
| **0.4.1 — Remote access & providers** (new HZN) | One transport, N tunnels; provider parity; release rail | Kanmer MCP serves Streamable HTTP with token auth; tunnel providers are adapters (cloudflared first; the OpenAI/ChatGPT tunnel remains a documented adapter); connector health is diagnosable; grok/Antigravity move to real plugin installs; the release pipeline can no longer ship broken assets. |
...
| G-REMOTE | epic | Remote access | Outcome: any remote MCP client can reach a local Kanmer board through one Streamable HTTP endpoint with token auth, via interchangeable tunnel adapters; health is diagnosable in one command. In scope: FRD/ADR, transport, auth, adapter contract + cloudflared, doctor, GUI lifecycle (GUI-095), ChatGPT-tunnel docs as one adapter manual (DOC-010). Out of scope: MCP-020 dispatch-over-MCP (separate authz boundary), OAuth (deferred), multi-board single endpoints. Risks: secrets handling (no plaintext), localhost-bind default, per-project isolation. Done: integration ticket proves a real remote client on a second machine. |
...
- **What:** `docs/manual/` chapter for remote access: the HTTP endpoint + token flow, the adapter model, per-provider connection walkthroughs (ChatGPT via the existing tunnel doc, Claude custom connector, generic MCP client), troubleshooting via the doctor. Regenerate the in-app manual (`build-manual.mjs`).
...
- **What:** end-to-end from a genuinely remote client (second machine or hosted agent): tunnel up via adapter → HTTP + token → full ticket read/write round-trip → doctor reports healthy → tear-down leaves no secrets on disk.
- **Verification:** proof is the command/interaction log from both machines.
...
**Existing tickets regroomed into 0.4.1:** MCP-021 (anchor, rescoped: tunnel **adapter contract** + first adapter cloudflared; the OpenAI/ChatGPT tunnel stays a documented manual adapter — blocked by S-16, S-17; blocks GUI-095, S-19); GUI-095 (tunnel lifecycle UI; blocked by MCP-021); MCP-020 (dispatch over MCP — in the horizon, **not** in G-REMOTE: separate authorization boundary); MCP-014 (grok plugin install); MCP-015 (Antigravity plugin + dispatch, gated on GUI-073 adjudication); MCP-008 (rescoped: headless serve + `.mcpb` for Claude Desktop; **unblocked** from archived MCP-005; relates S-17); GUI-075 (per-provider dispatch model + prompt); GUI-092 + GUI-093 (release rail single-pack + resilient publish; relate S-20); GUI-090 (staleness report in the GUI); GUI-084 (notification wording/theme); GUI-087 (friendly gate errors — the dead `friendlyGateError` never matches; make every gate refusal human); GUI-088 (managed AGENTS.md block for marketplace hosts too); DOC-008 (README corrections); DOC-009 (AGENTS.md repo-map corrections); GUI-068 (update-path verification — needs the next release; re-horizoned here); DOC-010 (merges this session; stays a G-REMOTE member as the ChatGPT adapter manual).
