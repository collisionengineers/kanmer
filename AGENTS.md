  ...
  areas/
    api/                # folder name = area id
      API-001/          # folder name = ticket id
        ...
        checklist.md    # │ the ticket may reach the board's final stage — and
        proof.md        # ┘ on a strict board it must be a valid PASS record.
...
The only place that touches `.kanmer` files. Public API via `index.ts`. Key entry point: **`KanmerStore`** (`store.ts`) — construct with a project root, then `listItems(WithWarnings)/getItem/createItem/updateItem/moveItem/deleteItem/searchItems`, `takeTicket/releaseTicket`, `getDoc/getDocWithVersion/setDoc/getTicketDocsInfo`, `getBoard(WithSource)/setBoard/addColumn/updateColumn/removeColumn/reorderColumns`, `detectFormat`, `getActivity`. `init()` maintains whichever format exists (it never stamps v2 onto a v1 board — that's `migrateToV2`'s job). Two record parsers sit beside the store and are the *only* readers of their documents' contents: `review-attestation.ts` (`scratch/review.md`) and `proof-record.ts` (`proof/proof.md`, CORE-129). The latter is what makes `proof-record/2` mean something — a non-empty typed `attempts[]` whose final entry is `authoritative`, with the top-level verdict bound to it — and how hard the Done gate enforces it is `board.yml`'s `proofValidation.mode`: absent resolves to `report` (existence plus warnings, unchanged), a board created today is written `strict` (a valid PASS or no move), and an existing board reaches strict only through `migrate_board`'s digest-bound census. Records without `schema: 2` are reported `legacy` and are never rewritten. Links live in `links.ts` (`getLinkGraph`, `linkItems`, `computeBlockedIds`, `parseWikiLinks`). Everything is covered by `*.test.ts` (vitest), including a v1 fixture suite and the migration round-trip.
...
- **Renderer imports from `@kanmer/core` must be `import type`.** Core pulls in Node-only deps (gray-matter, chokidar); the renderer is a browser context. Type-only imports are erased at build. Never import a runtime value from core in `renderer/`. Runtime helpers come from the browser-safe entry `@kanmer/core/browser` (`packages/core/src/browser.ts`) instead; `scripts/renderer-core-imports.test.mjs` enforces this by scanning every renderer file for a non-`type` import of `@kanmer/core`.
...
19. **Plan validation flags; only a `step` request blocks.** FRD-033 (CORE-118) adds two core modules — `plan.ts` (`parsePlan`/`validatePlan`, plus the ATX section reader that used to live in `execution-packet.ts`) and `step-packet.ts` (`compileStepPacket`, `step-packet/2`) — and one optional `step` parameter on `get_execution_packet`. There is **no new tool** (CORE-118 added none; the roster moved to 40 when CORE-131 added `apply_reconciliation` and to 41 when CORE-132 added `release_channel`), no new frontmatter field and no persisted packet; a packet is derived on read through core's bounded execution-authority snapshot. The severity split is load-bearing: `PLAN_VAGUE_INSTRUCTION` and `PLAN_RISK_EVIDENCE_MISSING` are permanently **advisory**, because the shipped plan template says the decision-verb warning "is not a gate or regex score" and `scripts/verify-skill-prose.mjs:347-348` pins that sentence — do not promote them. Structural findings (no compilable step, an allowed file the `## Expected files` table never declares, a file `## Do not modify` forbids, no usable acceptance check, no stop condition, evidence pinned to a superseded version) become blockers **only** when `step` is supplied, which is what keeps every existing plan's whole-ticket packet working unchanged. Only an exact level-three `### Step N — <title>` heading is a structured boundary: declared numbers start at 1 and remain contiguous, while nested or explanatory headings never become steps. The step refusal is strictly **last** in the refusal order and, like every other, writes nothing — `smoke.mjs` proves it by byte-comparing `.kanmer`, the ticket file and `activity.jsonl` across it. `"next"` reads a checklist by the step each box names (`- [ ] Step 2 — …`) and only falls back to box position, because real checklists carry more boxes than steps. Named checklist authority exists only when the checkbox label begins with `Step N`; an explanatory prose mention of `step N` never maps that checkbox to a step. A constrained packet is never minted unless the selected ordered step has at least one mapped unchecked checklist marker; a whole-ticket packet remains the setup route before that marker exists.
    **Constrained-step authority is controller-held and exact.** `step-packet/2` binds the full canonical packet with a 64-character SHA-256 digest. `packetId` is tamper-evident identity, not authentication. The trusted controller retains the exact full packet inside the live dispatch/reconciliation chain before handing it to a worker, reconciles only that retained object, and never accepts a worker-returned or reconstructed packet as authority. Do not persist full packets or prompts in the automation run ledger. If a crash or reconnect loses the retained object, record packet-loss as `INCONCLUSIVE`, issue no successor packet, and never rebuild authority from the worker result or current board.
    ...
    Git path evidence covers tracked, staged, unstaged and untracked paths plus both rename endpoints. Changed-path evidence also includes one bounded complete union of every path touched by every intervening commit, including paths later reverted; a non-ancestor baseline or exhausted history is `INCONCLUSIVE`. That history census validates both old and new modes from every intervening tree edge; any intervening `120000` symbolic-link or `160000` Git-link mode refuses even if a later commit restores a regular endpoint. A packet workspace HEAD is a full 40- or 64-character Git object ID. Every sample also hashes one bounded NUL `git ls-files -v -s -z` index census, binding flag, mode, object id, stage and path: assume-unchanged or skip-worktree entries refuse; nonzero stages and gitlinks refuse without index mutation; and census drift between samples is `INCONCLUSIVE`. On filesystems that expose the owner-executable bit, every clean tracked regular path must agree with its indexed `100644`/`100755` executable class; disagreement refuses. A tracked mode-`120000` path is retained only when its checkout representation and capped target bytes are identity-bound and its physical target is an indexed tracked regular file inside the worktree; external, chained-external, dangling, unreadable, unstable or over-budget links refuse. Tracked-link target bytes retain a leading UTF-8 BOM. Ignored or untracked link targets refuse. Execution authority requires exactly one selected ticket endpoint across v2 areas and legacy v1 storage; duplicates refuse before either record is opened. A non-traversable Windows placeholder is retained with its regular representation and exact bytes. Ignored paths and `.git` / common-directory metadata are outside that observable authority: constrained workers must never mutate them. Any need or attempt to do so is a deviation stop and the controller records `INCONCLUSIVE`; absence from the collected path list is never proof that such a write was safe.
...
24. **Claude Connect stages its marketplace in `%LOCALAPPDATA%\Kanmer\claude-marketplace` and proves the version it installed.** Claude Code records a *directory* marketplace by path, so the old `mkdtemp` staging that Connect deleted on the way out left every later session reporting `Marketplace kanmer failed to load: cache-miss` (GUI-147). That installer-owned root — the sibling of `%LOCALAPPDATA%\Kanmer\mcp` and `\bin`, resolved Node-side from `process.env.LOCALAPPDATA` because Electron's `app.getPath` has no `localAppData` key — is refreshed per owned subdirectory (`.claude-plugin`, `.agents`, `plugins/kanmer`) on every Connect and never deleted by Connect, a failed Connect, or Disconnect. `claude plugin install` is a **no-op on an already-installed plugin**, so add-vs-update and install-vs-uninstall+install are decided from the host's own `~/.claude/plugins/known_marketplaces.json` and `installed_plugins.json` (never from CLI text), a registration recorded anywhere other than the staged root is removed and re-added rather than `update`d, and Connect then requires `claude plugin list` to report the bundled `plugin.json` version — a mismatch or unreadable read-back fails Connect with the pasteable uninstall+install repair instead of a green tick. `skillsStatus("claude")` surfaces that same read (soft-failing to `null`), and Disconnect removes this provider's own plugin then its marketplace registration. Tests must never run a mutating `claude plugin …` command: stub `ConnectOptions.claudePluginStateDir`/`hostVersionRunner` and point `LOCALAPPDATA` at a temp directory.
...
- **Project registrations name only the installer-owned launcher; legacy absolute ones are reported.** Since GUI-149 every project registration Connect writes — `.mcp.json` (Claude Code), `.codex/config.toml` (Codex), `opencode.json` (OpenCode) — is the same `powershell.exe … & (Join-Path $env:LOCALAPPDATA 'Kanmer\bin\kanmer-mcp.cmd')` contract with `KANMER_BOARD_BRANCH` as the only variable (`PORTABLE_LAUNCHER_ARGS` in `packages/core/src/staleness.ts`, `portableLauncherInvocation()` in `providers.ts`). No `process.execPath`, bundled script path, `--root` or `--repo-root` is serialised, so a manual reinstall to another directory (`allowToChangeInstallationDirectory: true`) no longer strands them, and the board is discovered from the host's cwd (ADR-0012). Files written before GUI-149 still carry the absolute Electron shape; `get_status.repo.stale` reports them as `mcp-registration` `behind` on Windows (`isLegacyLauncherDescriptor`) with "reconnect this project" as the fix. Connect probes the launcher (`--probe`) before writing for all three hosts and refuses without an absolute-path fallback, which means an unpackaged dev build cannot Connect a project-file host — the same rule Codex has had since GUI-100. Connect also appends each host's registration file and skills copy to the *target project's* `.gitignore` (`connectIgnoreEntries()`), when the project has a `.git`; this repo's own `.gitignore` block below §8 lists the same paths and a providers test pins the two in agreement.
